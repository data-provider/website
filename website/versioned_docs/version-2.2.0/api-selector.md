...
* __`options`__ _(Object)_: An object containing options for the selector, which properties can be:
	...
	* __`getChildQueryMethod(query, parentQuery)`__ _(Function)_: When defined, this method will be used to get the resultant query value when the `query` method is called. The default method extends the parent query value with the provided query value _(recursively querying a selector applies also parent queries)_. Use this method is you want to apply a deep extend instead of a simple extend, for example. `getChildQueryMethod: (query, parentQuery) => _.merge({}, query, parentQuery);`
