---
id: version-2.1.2-api-providers-and-selectors-methods
...
Resets the `data`, `loading` and `error` state properties to its original state (the ones defined in the `initialState` option, or the default ones `{ data:undefined, error: null }` if no `initialState` was defined).
...
// { data: [{ title: "Don Quijote " }], loading: false, error: null}
...
_(Object)_: Current state of the provider or selector, containing `data`, `loading` and `error` properties.
